import { i as isAdmin, h as hasGlobalStaffRole } from './pages-RIbRYNlC.js';
import { h as handle$1 } from './auth-DhpCFoFY.js';
import { A as get_request_store, B as merge_tracing, C as with_request_store, r as redirect } from './index.js-H2yCcV1v.js';
...
var PUBLIC_PREFIXES = ["/login", "/auth"];
var PUBLIC_EXACT = /* @__PURE__ */ new Set(["/api/health"]);
var guardHandle = async ({ event, resolve }) => {
	const { pathname } = event.url;
	if (PUBLIC_EXACT.has(pathname) || PUBLIC_PREFIXES.some((p) => pathname.startsWith(p))) return resolve(event);
	const session = await event.locals.auth();
	if (!session?.user?.discordId) throw redirect(303, `/login?callbackUrl=${encodeURIComponent(pathname)}`);
	const roleIds = session.user.roleIds ?? [];
	const admin = isAdmin(session.user.discordId);
	if (!admin && !hasGlobalStaffRole(roleIds)) {
		if (pathname !== "/access-denied") throw redirect(303, "/access-denied");
	}
	if (pathname.startsWith("/admin") && !admin) throw redirect(303, "/access-denied");
...
var handle = sequence(handle$1, guardHandle);

export { handle };
//# sourceMappingURL=hooks.server-Bsz_kPso.js.map
